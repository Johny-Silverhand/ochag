...
1. **Environment**
   ...
   - Опционально `OCHAG_APP_ORIGIN`, если API когда-нибудь откроют с другого origin. Боевой хост `https://restopro.vercel.app` уже разрешён в коде; прежние алиасы `https://restopro-theta.vercel.app` и `https://ochag-theta.vercel.app` тоже.
