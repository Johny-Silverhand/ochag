...
type Panel = "about" | "plans" | "checkout" | "onboard" | null;
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-2 sm:grid-cols-2">
        <Button type="button" variant="outline" className="w-full" onClick={() => setPanel("about")}>
          Что это?
        </Button>
        {!onboarded ? (
          <Button type="button" variant="secondary" className="w-full" onClick={() => setPanel("plans")}>
            Выбрать тариф
          </Button>
        ) : null}
      </div>
      {canCreateNetwork ? (
        <Button type="button" className="w-full" onClick={() => setPanel("onboard")}>
          Создать сеть
        </Button>
      ) : null}
      ...
      <AboutDialog open={panel === "about"} onOpenChange={(open) => setPanel(open ? "about" : null)} />
      <PlansDialog
        open={panel === "plans"}
        selected={picked}
        onSelect={setPicked}
        onOpenChange={(open) => setPanel(open ? "plans" : null)}
        onCheckout={(id) => {
          setPicked(id);
          setPanel("checkout");
        }}
      ...
      <CheckoutDialog
        ...
        onOpenChange={(open) => setPanel(open ? "checkout" : null)}
        onBack={() => setPanel("plans")}
        onPay={async () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title={`Что такое ${NETWORK_NAME}`} className="max-h-[min(90dvh,44rem)] max-w-2xl overflow-y-auto">
        ...
        <p className="mt-4 text-xs text-muted">
          Учётку после оплаты создаёте сами. Уже выданный логин — вход справа, без повторной регистрации.
        </p>
      </DialogContent>
    </Dialog>
  );
...
function PlansDialog({
  ...
  onCheckout: (id: TariffId) => void;
}) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title="Тарифы Очаг" className="max-h-[min(92dvh,48rem)] max-w-3xl overflow-y-auto">
        ...
        <Button type="button" className="mt-4 w-full" onClick={() => onCheckout(selected)}>
...
function TariffCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "rounded-xl bg-elevated p-3 text-left shadow-(--shadow-border) transition-[box-shadow,transform] duration-150",
...
function CheckoutDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title="Оплата тарифа" className="max-h-[min(90dvh,40rem)] overflow-y-auto">
        ...
        <div className="mb-4 flex gap-3 rounded-lg bg-bg p-3">
          <img src={plan.image} alt="" className="size-16 rounded-md object-cover" />
          <div>
...
function OnboardDialog({
  ...
  async function create() {
    setBusy(true);
    setError("");
    const result = await submit({ ownerName, login, password, pin, branchName, city, address });
    setBusy(false);
    if (!result.ok) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent title="Создать сеть" className="max-h-[min(92dvh,46rem)] overflow-y-auto">
        <p className="mb-4 text-sm text-muted">
